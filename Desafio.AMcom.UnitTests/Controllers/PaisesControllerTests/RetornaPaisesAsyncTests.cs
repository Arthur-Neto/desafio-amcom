...
{
    public class RetornaPaisesAsyncTests : IDisposable
    {
        private readonly Mock<IMediator> _moqMediator;

        public RetornaPaisesAsyncTests()
        {
        ...
        public async Task Deve_Verificar_Metodo_E_Retornar_OkResult_Quando_Retorno_Do_Mediator_For_Sucesso()
        {
            ...
            _moqMediator
                .Setup(p => p.Send(It.IsAny<RetornarPaisesQuery>(), default))
                .ReturnsAsync(resultado);

            // Act
            var result = await GetController().RetornaPaisesAsync(default);
        ...
        public void Deve_Verificar_Atributos()
        {
            ...
            typeof(PaisesController)
                .GetMethod(nameof(PaisesController.RetornaPaisesAsync))
                .Should()
